...
Setting `--swatch-style: dot` renders a swatch as a small circular marker — handy when plotting swatches as points (this is how [`<hue-wheel>`](../hue-wheel) displays slotted swatches). It inherits, so setting it on a container switches a whole group at once. Size the dots with `--dot-size` (default `1em`, `1.5em` at `size="large"`), tweak their `--dot-border` / `--dot-shadow`, and pair it with `--details-style: compact` so each dot's details surface in a hover/focus tooltip:
...
| `--swatch-style`             | `dot` &#124; `normal` (default)                   | `dot` renders the swatch as a small circular marker (diameter `--dot-size`), clipping its contents — used when plotting it as a point (e.g. inside [`<hue-wheel>`](../hue-wheel)). Set it on the swatch's host/ancestor (it inherits, so one declaration switches a whole group); pair with `--details-style: compact` to surface details on hover/focus. |
| `--dot-size`                 | `<length>`                                        | In dot mode, the diameter of the dot. Defaults to `1em` (`1.5em` at `size="large"`).                                                                                                                                                                                                                                                                      |
| `--dot-border`               | `<line-width> &#124; <line-style> &#124; <color>` | In dot mode, the dot's border. Defaults to a white border that scales with `--dot-size`.                                                                                                                                                                                                                                                                  |
| `--dot-shadow`               | `<shadow>`                                        | In dot mode, the dot's `box-shadow`. Defaults to a soft drop shadow.                                                                                                                                                                                                                                                                                      |
